...
export const UserProvider = (props) => {
    ...
    try {
      ...
      if (existingItem) {
        ...
        const updateResponse = await fetch(
          `${API_URL}/cart/update/${existingItem._id}`,
          {
            method: 'PUT',
            headers: headers,
            body: JSON.stringify({
  ...
  const updateCartQuantity = async (cartItemId, newQuantity) => {
    if (!token || !cartItemId || newQuantity < 1)
  ...
  const clearCart = async (cartItemIds = []) => {
    try {
      ...
      if (response.status === 404) {
        console.log(
          'remove-multiple endpoint not found, trying fallback method...'
        )
      ...
      if (!response.ok) {
        throw new Error(
          data.message || `HTTP ${response.status}: Failed to clear cart`
        )
      }
      ...
      if (error.message.includes('JSON Parse error')) {
        return {
          success: false,
          message:
            'Server error: Invalid response format. Please check if the API endpoint exists.'
        }
      }
