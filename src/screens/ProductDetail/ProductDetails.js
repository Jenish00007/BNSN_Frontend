...
const ProductDetail = () => {
    ...
    const images = hasMultipleImages ? product.images : [product?.image || product?.images?.[0]];
    ...
    const getShopImageSource = () => {
        ...
        const shopImageUrl = product?.shop?.avatar ||
            product?.shop?.logo ||
            product?.shop?.image ||
        ...
        if (shopImageUrl && typeof shopImageUrl === 'string' && shopImageUrl.trim() !== '') {
            // Validate URL format
            try {
    ...
    const renderRatingStars = (rating) => {
        ...
        for (let i = 0; i < 5; i++) {
            if (i < fullStars) {
                stars.push(<FontAwesome key={i} name="star" size={20} color="#FFC107" />);
            } else if (i === fullStars && halfStar) {
                stars.push(<FontAwesome key={i} name="star-half-o" size={20} color="#FFC107" />);
            } else {
                stars.push(<FontAwesome key={i} name="star-o" size={20} color="#FFC107" />);
            }
        }
        return stars;
    };
    ...
    const handleChat = () => {
        ...
        if (product?.shop?._id) {
            navigation.navigate('Chat', {
                ...
                shopName: product.shop.name || 'Seller'
            });
        } else {
    ...
    const handleSubmitReview = async () => {
        ...
        if (!orderIdForReview) {
            Alert.alert("Error", "Unable to find your purchase record for this product");
            return;
        }
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: branding.backgroundColor }]}>
            ...
            <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false} style={styles.scrollView}>
                ...
                <View style={styles.mainImageContainer}>
                    <TouchableOpacity onPress={() => { setFullViewIndex(activeImageIndex); setFullViewVisible(true); }}>
                        <Image
                    ...
                    {product?.originalPrice > product?.discountPrice && (
                        <View style={styles.discountBadgeAbsolute}>
                            <Text style={styles.badgeText}>-{Math.round(100 * (product.originalPrice - product.discountPrice) / product.originalPrice)}%</Text>
                        </View>
                    )}
                    {/* Badges Row (left) */}
                    <View style={styles.badgeRow}>
                        {product?.bestSeller && (
                            <View style={[styles.badge, styles.bestSellerBadge]}><Text style={styles.badgeText}>Best Seller</Text></View>
                        )}
                        {product?.stock > 0 && product?.stock <= 10 && (
                            <View style={[styles.badge, styles.limitedStockBadge]}><Text style={styles.badgeText}>Limited Stock</Text></View>
                        )}
                    </View>
                </View>
                ...
                {finalImages.length > 1 && (
                    ...
                    >
                        {finalImages.map((img, idx) => (
                            <TouchableOpacity
                                key={idx}
                                style={[styles.thumbnailWrapper, activeImageIndex === idx && styles.selectedThumbnail]}
                ...
                <View style={[styles.infoSection, { backgroundColor: branding.secondaryColor }]}>
                    <View style={styles.nameAndPrice}>
                        <Text style={[styles.productName, { color: branding.textColor }]}> {product?.name} </Text>
                        {/* Ratings as stars below name */}

                        <View style={styles.priceContainer}>
                            {product?.originalPrice > product?.discountPrice && (
                                <Text style={[styles.originalPrice, { color: branding.textColor }]}> ₹{product?.originalPrice} </Text>
                            )}
                            <Text style={[styles.productPrice, { color: branding.primaryColor }]}> ₹{product?.discountPrice} </Text>
                        </View>
                        {/* Unit Count and Unit */}
                        {product?.unitCount && product?.unit && (
                            <Text style={styles.unitText}>{product.unitCount} {product.unit}</Text>
                        )}
                    </View>
                    ...
                    <View style={styles.descriptionContainer}>
                        ...
                        <Text style={[styles.productDescription, { color: branding.textColor }]}>
            ...
            <View style={[styles.bottomActions, { backgroundColor: branding.secondaryColor }]}>
                {/* Chat Button */}
                <TouchableOpacity
                    style={[styles.actionButton, { backgroundColor: branding.primaryColor }]}
                    onPress={handleChat}
                >
            ...
            >
                <View style={styles.fullViewOverlay}>
                    ...
                    <View style={styles.fullViewCounter}>
                        <Text style={styles.fullViewCounterText}>{fullViewIndex + 1} / {finalImages.length}</Text>
                    </View>
                    <TouchableOpacity style={styles.fullViewClose} onPress={() => setFullViewVisible(false)}>
