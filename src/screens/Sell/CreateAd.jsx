...
const CreateAd = () => {
  ...
  // Dynamic form state for all categories
  ...
  const validateForm = (formToValidate = formData) => {
    const isCarCategory = categories.find(c => c._id === formToValidate.category)?.name?.toLowerCase() === 'cars'
    
    if (!formToValidate.name.trim()) {
      Alert.alert('Error', 'Ad title is required')
      return false
    }
    
    if (!isCarCategory && !formToValidate.description.trim()) {
    ...
    if (isCarCategory) {
      ...
      if (!carFormData.carBrand.trim()) {
        Alert.alert('Error', 'Car brand is required')
        return false
      }
      if (!carFormData.carModel.trim()) {
        Alert.alert('Error', 'Car model is required')
        return false
      }
      if (!carFormData.manufacturingYear) {
        Alert.alert('Error', 'Manufacturing year is required')
        return false
      }
      if (!carFormData.fuelType) {
        Alert.alert('Error', 'Fuel type is required')
        return false
      }
      ...
      if (!carFormData.kilometersDriven || parseInt(carFormData.kilometersDriven) < 0) {
        Alert.alert('Error', 'Valid kilometers driven is required')
        return false
      }
      if (!carFormData.numberOfOwners) {
        Alert.alert('Error', 'Number of owners is required')
        return false
      }
      if (!carFormData.rcAvailable) {
        Alert.alert('Error', 'RC availability is required')
        return false
      }
      if (!carFormData.insuranceStatus) {
        Alert.alert('Error', 'Insurance status is required')
        return false
      }
      if (!carFormData.discountPrice || parseFloat(carFormData.discountPrice) <= 0) {
        Alert.alert('Error', 'Valid expected price is required')
        return false
      }
      if (!carFormData.priceType) {
        Alert.alert('Error', 'Price type is required')
        return false
      }
      if (!carFormData.city.trim()) {
        Alert.alert('Error', 'City is required')
        return false
      }
      if (!carFormData.area.trim()) {
        Alert.alert('Error', 'Area / Locality is required')
        return false
      }
      if (!carFormData.pincode.trim() || carFormData.pincode.length !== 6) {
        Alert.alert('Error', 'Valid 6-digit pincode is required')
        return false
      }
    } else {
      // Regular product validation
      // if (!formToValidate.discountPrice || parseFloat(formToValidate.discountPrice) <= 0) {
      //   Alert.alert('Error', 'Valid price is required')
      //   return false
      // }
      // if (!formToValidate.stock || parseInt(formToValidate.stock) < 0) {
  ...
  const checkPostCostAndRedirect = async (formToSubmit) => {
    try {
      const categoryName = categories.find(c => c._id === formToSubmit.category)?.name || ''
      ...
      console.log('Checking post cost for category:', categoryName)
      console.log('Using API_URL:', API_URL)
      console.log('Token available:', !!token)

      const response = await fetch(`${API_URL}/user-posts/check-post-cost`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      if (result.success) {
        if (result.canPostForFree) {
          // User can post for free, proceed with creation
          await handleSubmit(formToSubmit)
        } else {
          // User needs to pay, redirect to payment screen
          navigation.navigate('PaymentScreen', {
            categoryName,
            cost: result.postCost,
        ...
        Alert.alert('Error', result.message || 'Failed to check post cost')
        return false
      }
    } catch (error) {
      ...
      Alert.alert('Error', 'Failed to check post cost. Please try again.')
      return false
    }
  }

  const handleSubmit = async (formToSubmit = formData) => {
    if (!validateForm(formToSubmit)) return

    console.log('Submit clicked - localSellerData:', localSellerData)
    console.log('Form data to submit:', formToSubmit)
  ...
  // CreateAd.jsx - Updated return section
return (
  <KeyboardAvoidingView
  ...
  >
    ...
    <View style={[styles.header, { backgroundColor: branding.primaryColor }]}>
      ...
      <TouchableOpacity
        onPress={() => {
          if (formData.category && categoryFormData.name) {
            const combinedFormData = {
              ...formData,
              ...categoryFormData,
              name: categoryFormData.name || formData.name,
              description: categoryFormData.description || formData.description,
              discountPrice: categoryFormData.discountPrice || formData.discountPrice,
              category: formData.category
            }
            checkPostCostAndRedirect(combinedFormData)
          } else {
            Alert.alert('Error', 'Please select a category and fill in the required details')
          }
        }}
        disabled={saving}
        style={styles.headerButton}
    ...
    {/* Content Area - Without ScrollView wrapper */}
    <View style={styles.contentContainer}>
      ...
      {formData.category ? (
        <DynamicCategoryForm 
          ...
          onSubmit={async () => {
            // Handle category form submission by merging with main form data
            const combinedFormData = {
              ...formData,
              ...categoryFormData,
              name: categoryFormData.name || formData.name,
              description: categoryFormData.description || formData.description,
              discountPrice: categoryFormData.discountPrice || formData.discountPrice,
              category: formData.category // Keep the category from main form
            }
            
            // Check post cost and redirect to payment if needed
            await checkPostCostAndRedirect(combinedFormData)
          }}
        ...
        <View style={styles.categoryPrompt}>
          <Text style={styles.promptText}>Please select a category to continue</Text>
          <Text style={styles.promptSubtext}>Choose from the available categories above</Text>
        </View>
      )}
    </View>
            ...
            {categories.map((category) => (
              <TouchableOpacity
                ...
                style={[
                  styles.selectionItem,
                  {
                    backgroundColor: formData.category === category._id ? branding.primaryColor : 'white',
                    borderColor: branding.borderColor
                  }
                ]}
                onPress={() => {
              ...
              >
                <Text
                  style={[
                    styles.selectionItemText,
                    { color: formData.category === category._id ? 'white' : branding.textColor }
                  ]}
                >
                  {category.name}
                </Text>
