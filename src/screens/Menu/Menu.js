...
import { captureError, addBreadcrumb, withPerformanceMonitoring } from '../../utils/sentryUtils'
...
function Menu() {
  ...
  const { loadingOrders, isLoggedIn, profile, fetchCartItems } = useContext(UserContext)
  ...
  const isLocationPermissionDenied = location?.label === 'locationPermissionDenied'
  ...
  const brandingColors = { primaryColor, secondaryColor, accentColor, textColor, backgroundColor, headerColor, buttonColor };
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      // Supermarkets
      fetchPromises.push(
        fetch('https://6ammart-admin.6amtech.com/api/v1/stores/latest?type=all', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'zoneId': zoneId,
      ...
      // All products
      fetchPromises.push(
        fetch(`${API_URL}/product/get-all-products?page=1&limit=${ITEMS_PER_PAGE}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
      ...
      fetchPromises.push(
        ...
        .catch(error => console.error('Error fetching latest items:', error))
        .finally(() => setNearbyMarketsLoading(false))
      );

      // Nearby market offers
      fetchPromises.push(
        fetch('https://6ammart-admin.6amtech.com/api/v1/stores/top-offer-near-me', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'zoneId': zoneId,
      ...
      fetchPromises.push(
        ...
        .then(async (response) => {
          ...
          if (json?.success && Array.isArray(json?.events)) {
            const validEvents = json.events.filter(event =>
              event &&
              typeof event === 'object' &&
              event._id &&
  ...
  const loadMoreAllProducts = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/product/get-all-products?page=${nextPage}&limit=${ITEMS_PER_PAGE}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  return (
    <>
      <SafeAreaView
        ...
        <View style={[styles().flex, styles(currentTheme, brandingColors).screenBackground]}>
                ...
                {search ? (
                  <View style={styles().searchList}>
                    <FlatList
                      ...
                      keyExtractor={(item, index) => `search-${item?._id?.toString() || index}-${index}`}
                      refreshControl={
                        <RefreshControl
                      ...
                      switch (item.type) {
                          ...
                          return (
                            <>
                              <View style={styles().sectionHeader}>
                                <TextDefault style={styles().sectionTitle}>Brows Categories</TextDefault>
                                <TouchableOpacity
                        ...
                        case 'recommended':
                          return (
                            <>
                              <View style={styles().sectionHeader}>
                                <TextDefault style={styles().sectionTitle}>Fresh recommendations</TextDefault>
                                <TouchableOpacity
                                  onPress={() => navigation.navigate('AllPopularItems', { type: 'recommended' })}
                                  style={styles().viewAllButton}
                                >
                                  <Text style={[styles().viewAllText, { color: primaryColor }]}>
                                    View All
                              ...
                              {recommendedItemLoading ? (
                                ...
                                <View >
                                  <FlatList
                                    ...
                                    keyExtractor={(productItem, index) => `recommended-${productItem?._id?.toString() || index}-${index}`}
                                  />
                                </View>
                              )}
                            </>
                                ...
                                <View >
                                  <FlatList
                                    ...
                                    renderItem={({ item: productItem }) => (
                                      <Products item={productItem} horizontal={true} />
                                    )}
                                    keyExtractor={(productItem, index) => `offers-${productItem?._id?.toString() || index}-${index}`}
                                  />
                                </View>
                              )}
                            </>
                        ...
                        case 'events':
                          ...
                          return (
                            <>
                              <View style={styles().sectionHeader}>
                                <TextDefault style={styles().sectionTitle}>Flash Sale ⚡</TextDefault>
                                <TouchableOpacity
                                  onPress={() => navigation.navigate('AllPopularItems', { type: 'events' })}
                                  style={styles().viewAllButton}
                                >
                                  <Text style={[styles().viewAllText, { color: primaryColor }]}>
                                    View All
                                ...
                                <View >
                                  <FlatList
                                    ...
                                    renderItem={({ item: eventItem }) => (
                                      <Products item={eventItem} horizontal={true} />
                                    )}
                                    keyExtractor={(eventItem, index) => `events-${eventItem?._id?.toString() || index}-${index}`}
                                  />
                                </View>
                              )}
                            </>
                          ...
                          return (
                            <>
                              <View style={styles().sectionHeader}>
                                ...
                                <TouchableOpacity
                                  onPress={() => navigation.navigate('AllPopularItems')}
                                  style={styles().viewAllButton}
                                >
                                  <Text style={[styles().viewAllText, { color: primaryColor }]}>
                                    View All
                              ...
                              {popularItemLoading ? (
                                ...
                                <View >
                                  <FlatList
                                    ...
                                    keyExtractor={(productItem, index) => `popular-${productItem?._id?.toString() || index}-${index}`}
                                  />
                                </View>
                              )}
                            </>
                                    ...
                                    <>
                                      <View style={{
                                        ...
                                        {allproducts.map((productItem, index) => (
                                          <View key={`all-items-${productItem?._id?.toString() || index}-${index}`} style={{
                                            width: '48%',
                                            marginBottom: 10
                                          }}>
                                            <Products
